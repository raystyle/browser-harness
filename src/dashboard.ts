 ...
 *   6. event tail          — recent interesting CDP events (PEEKED, never
 *                            drained — x-monitor's trigger lives on them)
 *
...
async function collectInstance(name: string): Promise<InstanceRow> {
  ...
  // active tab + operable tabs (read-only evals on the instance)
  try {
    const cur = await evalOn<{ targetId: string; url: string; title: string } | null>(port,
      'return await current_tab()', 4000);
    const tabs = await evalOn<Array<{ targetId: string; url: string; title: string }>>(port,
...
function collectWorker(): { heartbeatAgeS: number | null; logTail: string[]; db: { total: number; earliest: string; latest: string } } {
  const out = { heartbeatAgeS: null as number | null, logTail: [] as string[], db: { total: 0, earliest: '', latest: '' } };
...
/** Interesting recent events per live instance (PEEK — never drain). */
async function collectEventTail(inst: InstanceRow): Promise<Array<{ seq: number; t: number; method: string; summary: string }>> {
  if (!inst.alive || !inst.connected) return [];
  try {
    const evs = await evalOn<Array<{ seq?: number; t?: number; method: string; params?: any }>>(inst.port,
      'return await __bh_meta("peek", {limit: 40, filter: ["Page.frameNavigated","Network.loadingFailed","Page.javascriptDialogOpening","Target.attachedToTarget"]})', 4000);
    return (evs ?? [])
      .filter(e => e.seq != null)
      .slice(-20)
      .map(e => {
        let summary = '';
        if (e.method === 'Page.frameNavigated') summary = String(e.params?.frame?.url ?? '').slice(0, 70);
        else if (e.method === 'Network.loadingFailed') summary = `${e.params?.errorText ?? ''} ${(e.params?.requestId ?? '')}`.slice(0, 70);
        else if (e.method === 'Page.javascriptDialogOpening') summary = String(e.params?.message ?? '').slice(0, 60);
        else summary = String(e.params?.targetInfo?.url ?? e.params?.targetInfo?.title ?? '').slice(0, 60);
        return { seq: e.seq!, t: e.t ?? 0, method: e.method.replace(/^(Page|Network|Target)\./, ''), summary };
      });
  } catch { return []; }
}
...
export type DashboardState = {
  ...
  events: Record<string, Array<{ method: string; summary: string; sessionId?: string }>>;
...
async function snapshot(): Promise<DashboardState> {
  ...
  for (const i of instances) events[i.name] = await collectEventTail(i);
  return { ts: new Date().toISOString(), port: DASHBOARD_PORT, instances, rmux: await collectRmux(), worker: collectWorker(), events, assets: collectAssets() };
}
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; }
  ...
  /* right rail: Notification Center vibrancy */
  .rail { flex: 1 1 25%; min-width: 320px; max-width: 460px; padding: 16px; display: flex; flex-direction: column; min-height: 0;
  ...
  tr.evtnew td { animation: slidein .38s ease; }
  ...
  .notif .nbody b.ntitle { display: block; font-size: 12.5px; font-weight: 600; margin-bottom: 2px; }
...
<div class="win">
  ...
  <div class="layout">
    ...
    <div class="main"><div class="grid" id="root"><div class="card">连接中…</div></div></div>
    <aside class="rail"><div class="railhead">日志消息</div><div id="rail"></div></aside>
  </div>
</div>
<script>
const dn = n => (n.startsWith('x-') && n !== 'x-core') ? 'x-core::' + n : n; // app::component namespacing
...
function render(s) {
  ...
  h += '<div class="card wide"><h3>工作实例 <span class="count">'+s.instances.length+'</span></h3><table>';
  ...
  for (const i of s.instances) {
    ...
    const state = i.alive ? pill('存活', 'ok') + ' ' + pill(i.connected ? '已连接' : '无 CDP', i.connected ? 'on' : 'warn')
    ...
    h += '<tr><td style="white-space:nowrap"><b>'+esc(i.name)+'</b></td>'+
         ...
         '<td>'+state+'</td>'+
         '<td class="mono" >'+(i.uptime != null ? Math.round(i.uptime/60) + ' 分钟' + (i.replacements ? '（自愈 ' + i.replacements + '）' : '') : '—')+'</td>';
  ...
  if (!byTarget.size) h += '<tr><td><span class="muted">（无已连接实例——浏览器未被任何 bh 实例附着）</span></td></tr>';
  h += '</table></div>';
  // rmux: sessions as labelled table rows
  const r = s.rmux || {};
  h += '<div class="card wide"><h3>后台任务 <span class="count">'+((r.sessions||[]).length)+'</span></h3><table>';
  ...
  for (const ses of (r.sessions||[])) {
    for (const p of (ses.panes||[])) {
      ...
      h += '<tr><td style="white-space:nowrap"><b class="mono">'+esc(dn(ses.name))+'</b></td>'+
  ...
  h += '</table></div>';
  // 右侧日志栏：应用状态子区（心跳/下次检测）+ 累加式日志横幅
  const w = s.worker || {};
  let rail = '';
  const hbOk = w.heartbeatAgeS != null && w.heartbeatAgeS < 30;
  // 累加历史：新条目追加进浏览器侧数组，最多留 100 条，显示最新 30 条
  if (!window.__logAll) window.__logAll = [];
  const lines = (w.logTail||[]);
  const fresh = [];
  {
    const entries0 = [];
  ...
  // 下次检测时间：最近一轮刷新时间 + 两轮最少间隔（60s）
  let nextProbe = '—';
  {
    const lastT = /\\[([0-9:]+)\\]/.exec(window.__logAll.filter(l => l.includes('刷新')).slice(-1)[0] || '')?.[1];
    if (lastT) {
      const [hh, mm, ss] = lastT.split(':').map(Number);
      const d = new Date(); d.setHours(hh, mm, ss + 60);
      nextProbe = d.toTimeString().slice(0, 8);
    }
  }
  rail += '<div class="stcard">'
       +  '<div class="sthead">x-core::x-monitor</div>'
       +  '<div class="strow"><span class="stlabel">心跳</span><span class="stval">'+(hbOk ? esc(w.heartbeatAgeS+' 秒前') : esc('异常 '+(w.heartbeatAgeS??'-')+' 秒'))+'</span></div>'
       +  '<div class="strow"><span class="stlabel">检测节奏</span><span class="stval">5 秒探测 · 触发后 10 秒内随机抓取</span></div>';
  // 库存与时间线直读 sqlite（w.db，采集层只读查询），不再依赖日志格式
  const zh = (iso) => iso ? new Date(iso).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false }) : '无';
  if (w.db && w.db.total > 0) {
    rail += '<div class="strow"><span class="stlabel">库存</span><span class="stval mono">'+esc(w.db.total)+' 帖</span></div>'
         +  '<div class="strow"><span class="stlabel">库内时间线</span><span class="stval mono" style="font-size:11px">'+esc(zh(w.db.earliest))+' 至 '+esc(zh(w.db.latest))+'</span></div>';
  }
  rail += '</div>';
  ...
  for (const e of window.__logAll.slice(-10).reverse()) {
    ...
    if (body.includes('刷新失败')) { cls='err'; icon=GLYPH_EXCL; title='刷新失败'; }
    ...
    // 刷新横幅只显三行指标；库存/时间线由状态卡承载，不在横幅重复
    if (cls === 'ok') detail = detail.split('\\n').slice(0, 3).join('\\n');
    const isNew = fresh.includes(e);
    rail += '<div class="notif '+cls+(isNew?' slidein':'')+'">'+
         '<div class="nicon">'+icon+'</div>'+
         '<div class="nbody"><div class="nhead"><span class="napp">x-monitor</span><span class="ntime">'+esc(time)+'</span></div>'+
         '<b class="ntitle">'+esc(title)+'</b>'+
         '<div class="ndetail">'+esc(detail.slice(0,220))+'</div></div></div>';
  }
  // events: ACCUMULATED browser-side (same model as the log rail), newest on top
  const EVENT_ZH = { frameNavigated: '页面导航', loadingFailed: '请求失败', javascriptDialogOpening: '弹出对话框', attachedToTarget: '附着' };
  if (!window.__evtAll) window.__evtAll = [];
  const freshEvt = [];
  for (const [name, evs] of Object.entries(s.events||{})) {
    for (const e of (evs||[])) {
      const key = name + '#' + e.seq;
      if (!window.__evtAll.some(x => x.key === key)) { window.__evtAll.push({ key, name, ...e }); freshEvt.push(key); }
    }
  }
  if (window.__evtAll.length > 150) window.__evtAll.splice(0, window.__evtAll.length - 150);
  h += '<div class="card wide"><h3>事件 <span class="count">'+window.__evtAll.length+'</span></h3><table>';
  h += '<tr><td class="th" style="width:100px">实例</td><td class="th" style="width:90px">时间</td><td class="th" style="width:110px">事件</td><td class="th">内容</td></tr>';
  for (const e of window.__evtAll.slice(-7).reverse()) {
    const time = e.t ? new Date(e.t).toTimeString().slice(0, 8) : '—';
    h += '<tr'+(freshEvt.includes(e.key)?' class="evtnew"':'')+'><td><b>'+esc(dn(e.name))+'</b></td>'+
         '<td class="mono muted" style="white-space:nowrap">'+esc(time)+'</td>'+
         '<td style="white-space:nowrap">'+esc(EVENT_ZH[e.method] ?? e.method)+'</td>'+
         '<td class="mono muted" style="word-break:break-all">'+esc(e.summary)+'</td></tr>';
  }
  if (!window.__evtAll.length) h += '<tr><td colspan="4"><span class="muted">（暂无事件——守护实例重启后开始累积）</span></td></tr>';
  h += '</table></div>';
