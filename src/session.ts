...
export class Session implements Transport {
  ...
  private openWs(wsUrl: string, timeoutMs: number): Promise<void> {
    return new Promise<void>((res, rej) => {
      ...
      // Node's undici WebSocket reports connect failures with an EMPTY
      // ErrorEvent.message, so `?? fallback` never fires there — use a
      // truthiness check instead.
      ws.addEventListener('error', (e) => {
        const m = (e as any)?.message as string | undefined;
        finish(new Error(`WS error: ${m ? m : 'connect failed (likely 403, permission not granted, or port closed)'}`));
      });
