...
- **x 监控与搜索拆为两个应用（D46）**：`x-intel` 收敛为唯一监控实例与命令（worker 的 rmux 会话名也从 `x-monitor` 对齐为 `x-intel`），`x-search` 独立成应用：本地库检索（不需要任何 daemon）与全量收割（自有按需 daemon，空闲自退）。旧口令 `bh x-intel search|harvest` exit 2 并直接给出 `bh x-search` 等效命令
- **SKILL 渐进分层重构（D45）**：分层模型按资产实名（primitives/interaction-skills/domain-skills/应用）各就各位；652 方法口径核定为 667 条声明减 15 条 redirect 别名（可复算验证命令与全量 56 域表落 `primitives/protocol.md`）；SKILL 新增配方索引（17 篇逐条）、协议域族路由、知识检索（rg/grep 反查三库）、十应用盘点
