#!/usr/bin/env node
/**
 * Codegen: reads protocol/browser_protocol.json + protocol/js_protocol.json,
 * writes src/generated.ts with full TypeScript types and a `bindDomains(transport)`
 * factory that returns `{ Page: { navigate(...), ... }, DOM: { ... }, ... }`.
 *
 ...
 *
 * Run with `npm run gen` (Node ≥22 executes the erasable-TS syntax directly).
 */

import { readFile, writeFile } from 'node:fs/promises';
import { fileURLToPath } from 'node:url';

const HERE = fileURLToPath(new URL('..', import.meta.url)); // repo root
...
async function loadDomains(): Promise<Domain[]> {
  ...
  for (const fn of ['browser_protocol.json', 'js_protocol.json']) {
    const data = JSON.parse(await readFile(`${HERE}/protocol/${fn}`, 'utf8')) as { domains: Domain[] };
    out.push(...data.domains);
  }
...
async function build() {
  ...
  const out: string[] = [];
  out.push(`/* eslint-disable */\n// AUTO-GENERATED by scripts/gen.ts. Do not edit by hand.\n// Run \`npm run gen\` to regenerate from protocol/browser_protocol.json + protocol/js_protocol.json.\n`);
