...
| D20 | 初始原语收敛为常驻面：supervisor（rmux 守护）、看板、default、page-detect 应用在初始化时自动启动（幂等）；x-core 改为按需，仅当伴随 x.com 使用时才启动 | 已采纳 | 第 1 轮（2026-09-07 用户指令；裁定 1A=supervisor 指现有 rmux 守护、2A=page-detect init 时自动 watch、3A=x-core 仅显式命令触发不自动探测） | admin.ts ensureCompanions 加入 page-detect watch rmux 会话；验证 rmux sessions = page-detect/x-monitor/x-supervisor 自动出现 |
| D21 | 核心守护应用 `supervisor-core`：把 x-intel 内嵌的 x-supervisor 抽成独立通用监督应用，有自己的应用监控信息卡与实时事件；统一监督多个常驻应用（page-detect、x-intel worker 等） | 已交付 | 第 1 轮（2026-09-07 用户指令「supervisor 单独应用 / 核心守护应用 / supervisor-core」+ 1~5 全要） | assets/apps/supervisor-core.mjs + ensureCompanions 拉起；验收 [实证: status 监督2应用/page-detect正常/x-intel未运行 + 应用卡出现] |
