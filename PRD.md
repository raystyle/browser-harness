...
| D30 | 升级轮换原语 `bh upgrade`：守护进程版本漂移检测自动化（任何 bh 命令启动时轻量比对 health.version，漂移 stderr 提示一行）+ 轮换显式（`bh upgrade` 默认 dry-run 打印计划，--yes 执行五步序列：x-intel 拆栈、companions 停、default 重生、dashboard 换新、x-intel 恢复；spawn 子进程复用既有 stop/start 语义；stopped 状态尊重不拉；终验 health.version 全对版）；包源默认 GitHub Release（不降级/离线降级）、--from 显式源（URL 下载 + 后台引导安装规避 Windows 文件锁）、--offline 只滚动本地 | 已交付 | 第 2 轮（2026-09-08 用户指令 + 裁定：检测自动+轮换显式、默认 Release 也支持离线） | 实机滚动 0.4.0 至 0.4.1 五步全绿；顺带修 Commander 选项三连环（KNOWN_COMMANDS 注册/action 参数语义/全局 flag 抢占）；tag v0.4.1 |
