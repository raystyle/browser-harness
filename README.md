...
bh headless start                              # 自起无头 Chrome 引擎（临时、隔离、用完即杀）
```
...
`web-fetch`（正文抓取）· `google/bing/medium-search`（两步契约搜索）· `page-detect`（页面被 Cloudflare/验证码拦截时弹窗告警）· `x-intel`（附着你已打开的 X 主页时间线监控入库）· `x-search`（X 本地库中文分词检索 FTS5+simple；全量收割自开专属 tab）· `super-ocr`（验证码图识别）· `cookie-io`（登录态迁移）· `supervisor-core`（常驻应用自愈），全部经 `bh <name>` 调用，墙与人机验证如实报告，不硬闯。
