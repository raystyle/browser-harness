...
```bash
npm install -g ./browser-harness-ts-0.5.1.tgz
bh --version   # -> 0.5.1
```
...
| 页面告警 | `bh page-detect watch [--interval S]`：后台盯住你所有页面，一旦被 Cloudflare 五秒盾/人机挑战、Google 验证码、登录墙、白屏卡死、资源被 CDN 拒绝等拦截，**立即弹 Chrome 系统通知告警**，你来窗口完成验证即恢复；`unwatch` 停，`bh page-detect [url片段]` 可单次查某一页 |
...
| page-detect | **页面被拦截时弹窗告警**：后台盯住你所有页面，撞上 Cloudflare 五秒盾/人机挑战、Google 验证码、登录墙、白屏卡死、资源被 CDN 拒绝时立即弹 Chrome 系统通知，你无需盯屏即可知道「该去窗口做人机验证了」；完成验证自动恢复、告警自动消退 | `bh page-detect watch` 开、`unwatch` 停、`[url片段]` 单次查某页 |
...
```
Run: 从 https://github.com/raystyle/browser-harness/releases 下载 browser-harness-ts-0.5.1.tgz，`npm install -g ./browser-harness-ts-0.5.1.tgz`，确认 `bh --status` 可用，然后用
