...
```bash
npm install -g ./browser-harness-ts-0.5.2.tgz
bh --version   # -> 0.5.2
```
...
```
Run: 从 https://github.com/raystyle/browser-harness/releases 下载 browser-harness-ts-0.5.2.tgz，`npm install -g ./browser-harness-ts-0.5.2.tgz`，确认 `bh --status` 可用，然后用
